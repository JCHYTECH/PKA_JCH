...
- Plusieurs fichiers sensibles sont lisibles par le groupe et les autres utilisateurs locaux (`0644`) : `.env`, bases SQLite, logs, backups.
- Le dashboard est servi par defaut sur `127.0.0.1:8787` avec token pour acces non local.
- Les backups SQLite existent dans `TEAM/backups/`.
- Aucun depot Git n'a ete detecte a la racine PKA_JCH.

## Outils deja disponibles

- `scripts/backup_team_db.py` : backup SQLite quotidien de `TEAM/team.db`, retention 30 jours.
- `scripts/dashboard_server.py` : dashboard local avec token, endpoints d'etat, lancement de modeles autorises, Dobby Live.
- `scripts/model_client.py` et `scripts/model_config.json` : routage multi-modeles et fichiers de cles dans `~/.config/pka-jch`.
- `scripts/telegram-bot/bot.py` : bot Telegram avec whitelist `TELEGRAM_USER_ID`, historique local SQLite, voix, TTS, vision.
...
- Outils CLI disponibles via l'environnement : `sqlite3`, `find`, `stat`, `rg`, scripts Python locaux.
...
| Bases SQLite lisibles localement | Eleve | `team.db`, `conversation.db`, backups | DB et backups en `0644` | `chmod 600` sur DB/backups, audit quotidien des permissions |
...
| Dependances Python anciennes ou vulnerables | Moyen | `requirements.txt` | Versions figees : anthropic/openai/httpx/telegram | Ajouter `pip-audit`, revision mensuelle, upgrades testes |
...
| Absence de depot Git detecte | Moyen | tout PKA | Pas d'historique versionne visible | Initialiser depot prive ou strategy de snapshots, `.gitignore` strict, hooks secrets |
...
- Mettre a jour `AGENTS.md` / `ROSTER.md` si Chouette est bien le 25e actif.
- Creer `.gitignore` strict avant toute mise sous Git.
