...
|---|---|---|---|---|
| Dobby 🦉 | Chargement du contexte de session | À chaque démarrage de session | Lecture de `MEMORY.md`, scan de `JCH_Inbox/00_INBOX/`, confirmation d’activation | JCH |
...
| Dobby 🦉 | Auto-processing inbox | À chaque fichier dans `JCH_Inbox/00_INBOX/` | Lecture, log dans `file_index`, routage, retour | Forge si technique, spécialiste concerné sinon |
| Dobby 🦉 | Capture de procédure | Après toute tâche complexe résolue | Indexation de la procédure dans `skills` | `TEAM/team.db` |
| Dobby 🦉 | Mise à jour mémoire structurelle | Quand une règle ou un état durable change | Mise à jour de `MEMORY.md` et cohérence système | `MEMORY.md`, `memory_log` |
| Sybil 🦔 | Entrée journal guidée | Quotidien ou à chaque session journal | Entrée dans `wiki/Daily/YYYY/MM/YYYY-MM-DD-slug.md` + structure journal | JCH, table `journal` |
| Sybil 🦔 | Continuité de l’entrée du jour | Si une entrée du jour existe déjà | Ajout d’une section, sans créer un second fichier | `wiki/Daily/...` |
...
| Forge 🦦 | Maintenance dashboards HTML | Après tout changement structurel | Mise à jour de `01_DASHBOARDS/hub.html` et `organigramme.html` | Système |
...
| Corbeau 🐦‍⬛ | Curation connaissance | À chaque output de connaissance livré par un spécialiste | Validation, connexion, classement dans la base de connaissance | `knowledge`, `knowledge_links` |
...
| Bouvier 🐕 | Recrutement / onboarding | Après brief Furet validé | Nouveau membre dans `team.db`, responsabilités, fiche identité | Dobby puis système |
