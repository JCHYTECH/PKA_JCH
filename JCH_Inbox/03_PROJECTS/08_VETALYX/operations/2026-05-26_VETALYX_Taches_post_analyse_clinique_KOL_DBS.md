---
date: 2026-05-26
project: VETALYX
type: task-list
status: active
owner: [[Dobby]]
source:
  - market evidence pack
  - clinical review
  - 6-line rapid test specification
  - KOL validation protocol
  - DBS flowcharts
---

# VETALYX - Liste de taches post-analyse clinique, KOL et DBS

## 1. Produit - tests rapides 6 lignes

| Priorite | Tache | Responsable propose | Livrable |
|---|---|---|---|
| P0 | Valider la logique "ligne mixte" avec DIXUN : une ligne = groupe allergenique, pas allergene individuel | JCH + [[Dobby]] | Question technique envoyee |
| P0 | Demander a DIXUN si les lignes mixtes sont techniquement faisables : acariens, storage mites, pollens, moisissures, puce | JCH + DIXUN | Reponse fournisseur |
| P0 | Confirmer les matrices acceptées par le test rapide : sang total, serum, plasma | DIXUN | IFU technique |
| P0 | Obtenir les donnees de validation chien et chat separees pour les tests rapides existants | DIXUN | Dossier validation |
| P0 | Demander la composition exacte des extraits : natifs, recombinants, mixes, concentrations, controle lot a lot | DIXUN | Fiche technique allergenes |
| P1 | Definir deux versions pollens : Nord/Ouest Europe et Mediterranee | [[Vasco]] + [[Clio]] | Panel regional v1 |
| P1 | Revoir le panel alimentaire comme P2 uniquement, avec claim "support de sensibilisation" | [[Vasco]] + [[Renard]] | Claim et IFU wording |

## 2. Rapport partenaire

| Priorite | Tache | Responsable propose | Livrable |
|---|---|---|---|
| P0 | Relire le rapport final DOCX pour correction style/orthographe avant envoi externe | JCH + [[Dobby]] | Version finale distribuee |
| P0 | Ajouter logo/identite Vetalyx si necessaire | [[Dobby]] | DOCX final brandé |
| P0 | Exporter en PDF apres verification visuelle dans Word/Pages/Google Docs | JCH | PDF partenaire |
| P1 | Preparer une version courte 2 pages pour email KOL/fournisseur | [[Dobby]] + [[Vasco]] | Brief KOL 2 pages |
| P1 | Preparer une version anglaise pour DIXUN | [[Dobby]] | English supplier brief |

## 3. KOL - Dr Morgane Cannone et Dr Billen

| Priorite | Tache | Responsable propose | Livrable |
|---|---|---|---|
| P0 | Confirmer les coordonnees directes des deux KOL | JCH | Contacts complets |
| P0 | Screener chaque KOL avant contact : carriere, specialite, publications, reseaux, prises de parole, contraintes institutionnelles, centres d'interet cliniques | [[Furet]] + [[Dobby]] | Fiche KOL personnalisee |
| P0 | Verifier si le contact "Dr Billen" correspond bien a Dr Frederic Billen, ULiege/CVU, medecine interne/gastroenterologie, ou a un autre Billen | [[Furet]] + JCH | Identification confirmee |
| P0 | Adapter l'angle d'approche a chaque KOL : dermatologie/allergologie, medecine interne, recherche clinique, biobanque, innovation diagnostique | [[Dobby]] + [[Vasco]] | Message personnalise |
| P0 | Envoyer un email d'approche avec le protocole pilote | JCH + [[Dobby]] | Email envoye |
| P0 | Proposer un call de 30 minutes a chaque KOL | JCH | Rendez-vous |
| P0 | Demander leur volume mensuel de cas allergologie chien/chat | KOL | Estimation recrutement |
| P0 | Demander leur test de reference habituel : ELISA, PAX/microarray, immunoblot, IDT | KOL | Choix comparateur |
| P1 | Verifier exigences comite ethique/institutionnel Maisons-Alfort et ULg | [[Renard]] + KOL | Avis ethique requis ou non |
| P1 | Definir compensation par cas complet et honoraires KOL | [[Bruno]] + JCH | Budget KOL |
| P1 | Valider la faisabilite chat : recrutement, sedation/prelevement, suivi | [[Vasco]] + KOL | Faisabilite chat |

## 3bis. Screening KOL - donnees a collecter avant approche

Objectif : personnaliser la demarche pour chaque opinion leader et eviter un contact generique. Le screening doit rester professionnel, base sur sources publiques et utile au positionnement scientifique/clinique.

| Axe | Questions | Utilisation |
|---|---|---|
| Parcours | Formation, poste actuel, service, diplome/specialisation, role universitaire | Adapter le niveau scientifique et institutionnel du message |
| Domaine clinique | Dermatologie, allergologie, medecine interne, gastroenterologie, feline medicine, otologie, immunologie | Positionner le protocole sur ses vrais sujets d'interet |
| Publications | Articles, theses encadrees, communications, chapitres, cas cliniques | Citer un interet commun et proposer co-valorisation |
| Reseaux | AFVAC, ESVD/ECVD, ECVIM, ENV, ULiege/CVU, groupes de formation | Identifier le levier de credibilite et diffusion |
| Prises de parole | Congres, formations, webinars, enseignement | Evaluer appetence pour projet KOL/formation |
| Contraintes | Comite ethique, charge clinique, statut hospitalier/universitaire, circuit contrats | Anticiper delais et documents |
| Passions/interets professionnels | Pedagogie, innovation diagnostique, cas complexes, chats, dermatologie, medecine interne, recherche translationnelle | Personnaliser le pitch sans etre intrusif |
| Potentiel Vetalyx | Site recruteur, conseiller scientifique, co-auteur rapport/poster, testeur DBS, relais reseau | Definir le role propose |

### Premiers elements publics identifies

| KOL | Elements observes | Angle d'approche preliminaire |
|---|---|---|
| Dr Morgane Cannone / Canonne-Guibert | Source publique Edimark : veterinaire a l'Ecole nationale veterinaire d'Alfort, unite pedagogique de medecine interne ; publications recentes sur la PIF feline. CV public trouve avec email ENVA. | Ne pas la cadrer uniquement dermatologie : proposer un angle medecine interne/feline medicine, validation clinique rigoureuse, echantillons conserves, chats et diagnostic differentiel. |
| Dr Billen | Source publique CVU Liege : Dr Frederic Billen, specialiste medecine interne petits animaux, service gastroenterologie ; les allergies alimentaires et inflammations digestives font partie du champ clinique. | Verifier l'identite exacte avant contact. Si Frederic Billen : angle medecine interne/gastroenterologie, adverse food reactions, diagnostic differentiel, comparaison IgE versus clinique. |

Sources de screening initial :

- Dr Morgane Canonne-Guibert, Edimark : https://www.edimark.fr/auteurs/canonne-guibert-morgane
- CV Morgane Canonne-Guibert public PDF : https://www.vetevent.be/files/cv_canonne_gilbert_morgane_2024.pdf
- CVU Liege gastroenterology, Dr Frederic Billen : https://www.cvu.uliege.be/cms/c_4836442/en/cvu-disciplines-gastroenterology
- CHUVA Maisons-Alfort informations pratiques : https://www.vet-alfort.fr/services-cliniques/

## 4. Protocole clinique

| Priorite | Tache | Responsable propose | Livrable |
|---|---|---|---|
| P0 | Transformer le protocole actuel en synopsis 4 pages | [[Dobby]] + [[Vasco]] | Synopsis KOL |
| P0 | Creer CRF consultation 1 page | [[Vasco]] + [[Trace]] | CRF v1 |
| P0 | Creer grille de lecture test rapide | DIXUN + [[Vasco]] | Grille interpretation |
| P0 | Creer plan d'analyse statistique court | [[Sigma]] | SAP v1 |
| P1 | Definir diagnostic composite pour analyse exploratoire sensibilite/specificite | [[Vasco]] + [[Clio]] | Definition endpoint |
| P1 | Definir bras controles non allergiques | [[Vasco]] | Criteres controles |
| P1 | Definir follow-up 4-8 semaines : contenu, modalites, charge KOL | [[Vasco]] + [[Trace]] | Follow-up form |

## 5. Echantillons serum/plasma

| Priorite | Tache | Responsable propose | Livrable |
|---|---|---|---|
| P0 | Choisir le laboratoire de reference IgE chien/chat | JCH + [[Vasco]] | Labo selectionne |
| P0 | Confirmer type tube, volume minimal, delai, temperature et conditions d'expedition | Laboratoire | SOP echantillons |
| P0 | Verifier acceptation serum vs plasma | Laboratoire | Matrice validee |
| P1 | Definir biobanque : duree conservation, proprietaire echantillons, destruction | [[Renard]] + JCH | Politique echantillons |
| P1 | Prevoir fiches de non-conformite : hemolyse, volume insuffisant, delai, temperature | [[Trace]] | Formulaire deviation |

## 6. Sous-protocole DBS / buvard avec partenaire chinois

| Priorite | Tache | Responsable propose | Livrable |
|---|---|---|---|
| P0 | Envoyer a DIXUN la question DBS : assay compatible eluats DBS oui/non | JCH + [[Dobby]] | Reponse DIXUN |
| P0 | Demander leur protocole d'extraction DBS si disponible | DIXUN | SOP extraction |
| P0 | Confirmer papier recommande : Whatman 903 ou autre support valide | DIXUN + [[Clio]] | Choix carte DBS |
| P0 | Definir volume par spot : 20/40 uL analytique ou 50-75 uL pratique | [[Clio]] + DIXUN | Specification depot |
| P0 | Confirmer conservation basse temperature : 2-8 C apres sechage, -20 C >72 h, -80 C biobanque | [[Clio]] + [[Trace]] | SOP stockage DBS |
| P0 | Definir tests de stabilite : J0/J3/J7/J14/J28, 2-8 C vs -20 C, stress temperature ambiante | [[Clio]] + DIXUN | Plan stabilite |
| P1 | Definir criteres go/no-go DBS : exploitables >90 %, concordance >=80 %, faibles positifs | [[Vasco]] + [[Sigma]] + [[Clio]] | Decision matrix |
| P1 | Prevoir photo/scan obligatoire du spot et indicateur humidite | [[Trace]] | Controle qualite DBS |
| P1 | Tester full spot vs punch si volume controle | DIXUN + [[Clio]] | Comparaison extraction |

## 7. Documents juridiques et proprietaires

| Priorite | Tache | Responsable propose | Livrable |
|---|---|---|---|
| P0 | Rediger notice information proprietaire | [[Renard]] | Notice v1 |
| P0 | Rediger consentement participation et prelevement | [[Renard]] | Consentement v1 |
| P0 | Ajouter consentement separe conservation/utilisation secondaire echantillons | [[Renard]] | Clause biobanque |
| P0 | Definir RGPD : donnees pseudonymisees, cle chez KOL, Vetalyx sans identite proprietaire | [[Renard]] | Note RGPD |
| P1 | Rediger convention KOL/site | [[Renard]] + [[Bruno]] | Convention v1 |
| P1 | Definir clauses publication, donnees agregées, confidentialite, conflit d'interets | [[Renard]] | Annexes contrat |

## 8. Logistique terrain

| Priorite | Tache | Responsable propose | Livrable |
|---|---|---|---|
| P0 | Creer kit terrain KOL : tubes, cartes DBS, dessicants, indicateurs humidite, enveloppes, etiquettes | [[Trace]] + [[Bruno]] | Kit KOL |
| P0 | Definir circuit expedition serum/plasma sous froid | [[Trace]] | SOP transport |
| P0 | Definir circuit expedition DBS sous froid | [[Trace]] | SOP transport DBS |
| P1 | Verifier classification transport : Exempt Animal Specimen vs UN3373 selon contexte | [[Renard]] + transporteur | Avis transport |
| P1 | Prevoir tracking temperature pour les premiers envois | [[Trace]] | Tracking log |
| P1 | Monitorer les 10 premiers cas pour ajuster SOP | [[Vasco]] + [[Trace]] | Rapport monitoring |

## 9. Budget et arbitrages

| Priorite | Tache | Responsable propose | Livrable |
|---|---|---|---|
| P0 | Construire budget 50, 100 et 160 animaux | [[Bruno]] | Budget scenarios |
| P0 | Chiffrer cout test de reference par animal | [[Bruno]] + laboratoire | Cout comparator |
| P0 | Chiffrer compensation KOL par cas complet | [[Bruno]] + JCH | Offre KOL |
| P1 | Chiffrer surcout DBS basse temperature | [[Bruno]] + [[Trace]] | Budget DBS |
| P1 | Definir seuil budget go/no-go phase pilote | JCH + [[Bruno]] | Decision budget |

## 10. Prochaines decisions JCH

1. Valider si le protocole KOL vise d'abord 50, 100 ou 160 animaux.
2. Choisir la langue du dossier KOL initial : francais uniquement ou francais + anglais.
3. Decider si DIXUN recoit le rapport complet ou seulement le sous-protocole DBS + questions techniques.
4. Choisir le test de reference cible : PAX/Nextmune, ELISA labo, immunoblot, autre.
5. Decider si le rapport final tests rapides doit etre converti en PDF avant envoi.
